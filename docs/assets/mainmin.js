...
var px = new pixelit({from: document.getElementById('pixelitimg'),  });
var paletteList = [
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var pixelit = async function pixelit() {
    ...
    if (px.drawto.height == 1) {
      document.querySelector('#changepalette').disabled = true;
      document.querySelector('#downloadimage').disabled = true;
    } else {
      document.querySelector('#changepalette').disabled = false;

    }

    await greyscale.checked ? px.convertGrayscale() : null;
    await palette.checked ? px.convertPalette() : null;
    await maxheight.value ? px.setMaxHeight(maxheight.value).resizeImage() : null;
    await maxwidth.value ? px.setMaxWidth(maxwidth.value).resizeImage() : null;
  ...
  changePalette.addEventListener('click', function (e) {
    if (currentPalette < (maxPalette - 1)) {
      currentPalette ++;
    } else {
      currentPalette = 0;
    }
    document.querySelector('#counter').innerHTML = '';
    var counp = document.createElement('p');
    counp.innerText = `${currentPalette + 1}`;
    document.querySelector('#counter').appendChild(counp);
  ...
  downloadimage.addEventListener('click', async (e) => {
    //download image
    // let dataimage = await px.getObjectUrl()
    // let transaction = await arweave.createTransaction({ data: dataimage,}, );
    // transaction.addTag('Content-Type', 'image/png');
    // console.log(transaction);
    // let datatransaction = await window.arweaveWallet.sign(transaction,)
    // console.log(datatransaction);
    //
    // const response = await arweave.transactions.post(datatransaction);
